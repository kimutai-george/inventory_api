...
class ShopView(ModelViewSet):
    queryset = InventoryGroup.objects.select_related('created_by')
    serializer_class = ShopSearializer
    permission_classes = (IsAuthenticatedCustom,)
    pagination_class = CustomPagination
...
class InvoiceView(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if keyword:
            search_fields = ("created_by__fullname","created_by__email","shop__name")
            query = get_query(keyword,search_fields)
            return results.filter(query)
...
class SalePerfomanceView(ModelViewSet):
    http_method_names = ('get',)
    permission_classes = (IsAuthenticatedCustom,)
    queryset = InventoryView.queryset
    ...
    def list(self,request,*args,**kwargs):
        ...
        data = InventoryWithSumSerializer(items,many=True).data
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not total:
            ...
            if monthly:
                ...
                shop = query.annotate(amount_total=Sum(
                    F("sale_shop__invoice_items__quantity") *
                    F("sale_shop__invoice_items__amount")
                )).order_by("-amount_total")

            data = ShopWithAmountSerializer(shop,many=True).data



class PurchaseSummaryView(ModelViewSet):
    http_method_names = ('get',)
    permission_classes = (IsAuthenticatedCustom,)
...
class InventoryCSVLoaderView(ModelViewSet):
    ...
    def create(self,request,*args,**kwargs):
        try:
            ...
            raise Exception("kindly provide CSV data")
